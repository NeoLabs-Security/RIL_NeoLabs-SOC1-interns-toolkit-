...
# Keep the host source owner-only (0700 directory / 0600 files). A one-shot root
# helper copies only the collector inputs into a Docker volume and transfers
# ownership to the image's non-root collector identity. The long-running service
# mounts the resulting volume read-only and still drops every Linux capability.
docker run --rm \
  --user 0:0 \
...
# Prove the actual non-root image identity can traverse/read the staged secrets.
docker run --rm \
...
printf '[OK] Private VCC collector credentials are staged without loosening host secret permissions.\n'
