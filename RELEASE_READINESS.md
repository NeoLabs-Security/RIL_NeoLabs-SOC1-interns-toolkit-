...
| Windows root startup | Ready | `START-NEOLABS-SOC.cmd` is the sole normal Windows entry point |
| Linux/Ubuntu root startup | Ready by source/contract; runtime depends on host | `start-neolabs-soc.sh` owns Ubuntu/Debian package/Docker/kernel/Wazuh setup and subsequent startup |
| Root repository UX | Ready | legacy setup/Docker/Doctor/CLI wrappers removed from root; implementation moved under `internal/`/existing runtime folders |
...
| Rule loading | Ready | startup reloads the Wazuh manager after current NeoLabs rules are asserted |
...
| Doctor diagnostics | Ready | same root launcher with `doctor` action checks auth → telemetry → raw file → rules → Filebeat → indexer → dashboard |
...
After first-run executable normalisation:
...
The launchers reuse existing local Wazuh secrets/configuration on later runs. Students should not delete `.env`, delete volumes or reconstruct the setup from low-level scripts during ordinary troubleshooting.

## Verification boundary

CI can verify Bash syntax, Linux launcher contract/source behaviour, Wazuh/Compose configuration and Windows PowerShell/CMD contracts. A GitHub-hosted runner is not a substitute for every intern's actual Windows/WSL/Docker Desktop or fresh Ubuntu Server host. The first real machine in each platform profile should still be used as a smoke-test workstation before broad rollout.
...
- A headless Linux server has no GUI browser; the launcher prints an SSH loopback port-forward path instead.
...
Do not use a workstation for cohort evidence when the toolkit cannot verify assigned-pod telemetry in the indexer, another pod appears, credentials/private keys are exposed, Wazuh administrative services are publicly exposed, real/production data appears or service instability is observed.
